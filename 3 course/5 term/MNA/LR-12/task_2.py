...
def task_proc(n, eps):
    ...
    while True:
        ...
        a = [(1 - math.exp(-2 * x) * h / 2) for x in points]
        b = [(16/(1 + x ** 2)) * (h ** 2) - 2 for x in points]
        c = [(1 + math.exp(3 * x) * h / 2) for x in points]
        d = [(2 - x ** 2) * math.exp(-2 * x) * (h ** 2) for x in points]
...
def task_2():
    points_1, currect_1, h_1 = task_proc(10, 0.05)
    points_2, currect_2, h_2 = task_proc(10, 0.005)
    points_3, currect_3, h_3 = task_proc(10, 0.0005)
